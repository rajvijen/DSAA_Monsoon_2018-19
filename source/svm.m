...
%     svmModel{k} = fitcsvm(dataset(idx,:), g(idx), ...
%         'BoxConstraint', Inf, 'Kernel_Function', 'rbf', 'rbf_sigma', 14.51);

    svmModel{k} = fitcsvm(dataset(idx,:), g(idx), ...
        'BoxConstraint', 2e-1, 'KernelFunction', 'linear', 'KernelScale', 14.51);

    %# test
    predTest(:,k) = predict(svmModel{k}, dataset(testIdx,:)); % matlab native svm function
end
...
for k = 1:numel(svmModel)
    %# test
    predQueryImg(:, k) = predict(svmModel{k}, queryImageFeatureVector); % queryImage = x.jpg, row=x from dataset
end
